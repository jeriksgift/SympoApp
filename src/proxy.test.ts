...
/**
 * The gate is the whole point of this file, so it is tested through `middleware()`
...
describe("middleware gating, logged out", () => {
  it("bounces /shiftverse to the entry page", async () => {
    const res = await middleware(req("/shiftverse"));
  ...
  it("bounces a nested shiftverse path too, not just the root", async () => {
    const res = await middleware(req("/shiftverse/result"));
    expect(res.status).toBe(307);
  ...
  it("still gates the pre-existing events", async () => {
    for (const p of ["/quiz", "/ctf", "/hunt", "/code"]) {
      expect((await middleware(req(p))).status).toBe(307);
    }
  });

  it("leaves the entry page itself reachable", async () => {
    const res = await middleware(req("/enter"));
